...
### use TrakerMuon||GlobalMuon instead of TrakerMuon for RECO
############## samples ################
...
config_id=tnpConfig(
    ...
    mass="pair_mass_cor",
    bins=binnings['ID'],
    expr='tag_IsoMu24 && tag_pt_cor>27 && tag_isTight && tag_PFIsoTight && tag_q*probe_q<0 && probe_isLoose',
...
config_trigger=config_id.clone(
    ...
    expr='tag_IsoMu24 && tag_pt_cor>27 && tag_isTight && tag_PFIsoTight && tag_q*probe_q<0 && probe_isMedium && probe_TkIsoLoose',
    test='probe_IsoMu24',
    method='count',
    systematic=[
...
config_reco=tnpConfig(
    ...
    test='probe_isTracker||probe_isGlobal',
    hist_nbins=98,
    hist_range=(52,150),
    method='fit',
    fit_parameter= fit_nominal,
...
Configs["2018_TrackerMuonOrGlobalMuon"]=config_reco.clone(
    data=samples_won['data2018'],
    sim=samples_won['mi2018'],
)

## RECO tests
Configs["2016b_TrackerMuonOrGlobalMuon_pt"]=Configs["2016b_TrackerMuonOrGlobalMuon"].clone(
    bins=[
        { 'var' : 'probe_eta' , 'type': 'float', 'bins': [round(i/20.,2) for i in range(-48,49)], 'title':'#eta' },
        { 'var' : 'probe_pt' , 'type': 'float', 'bins': [35, 70], 'title':'p_{T} [GeV]' },
    ],
)
Configs["2016b_TrackerMuonOrGlobalMuon_pt_iso"]=Configs["2016b_TrackerMuonOrGlobalMuon"].clone(
    bins=[
        { 'var' : 'probe_eta' , 'type': 'float', 'bins': [round(i/20.,2) for i in range(-48,49)], 'title':'#eta' },
        { 'var' : 'probe_pt' , 'type': 'float', 'bins': [35, 70], 'title':'p_{T} [GeV]' },
    ],
    expr=Configs["2016b_TrackerMuonOrGlobalMuon"].expr+" && probe_relIso04<0.15",
)
Configs["2016b_TrackerMuonOrGlobalMuon_pt_dz"]=Configs["2016b_TrackerMuonOrGlobalMuon"].clone(
    bins=[
        { 'var' : 'probe_eta' , 'type': 'float', 'bins': [round(i/20.,2) for i in range(-48,49)], 'title':'#eta' },
        { 'var' : 'probe_pt' , 'type': 'float', 'bins': [35, 70], 'title':'p_{T} [GeV]' },
    ],
    expr=Configs["2016b_TrackerMuonOrGlobalMuon"].expr+" && abs(pair_dz)<0.05",
)
Configs["2016b_TrackerMuonOrGlobalMuon_pt_iso_dz"]=Configs["2016b_TrackerMuonOrGlobalMuon"].clone(
    bins=[
        { 'var' : 'probe_eta' , 'type': 'float', 'bins': [round(i/20.,2) for i in range(-48,49)], 'title':'#eta' },
        { 'var' : 'probe_pt' , 'type': 'float', 'bins': [35, 70], 'title':'p_{T} [GeV]' },
    ],
    expr=Configs["2016b_TrackerMuonOrGlobalMuon"].expr+" && probe_relIso04<0.15 && abs(pair_dz)<0.05",
)
Configs["2016b_LooseID_pt_iso_dz"]=Configs["2016b_TrackerMuonOrGlobalMuon"].clone(
    bins=[
        { 'var' : 'probe_eta' , 'type': 'float', 'bins': [round(i/20.,2) for i in range(-48,49)], 'title':'#eta' },
        { 'var' : 'probe_pt' , 'type': 'float', 'bins': [35, 70], 'title':'p_{T} [GeV]' },
    ],
...
Configs["2016b_LooseID"]=Configs["2016b_TrackerMuonOrGlobalMuon"].clone(
    test="probe_isLoose",
)
Configs["2016b_LooseID_pt"]=Configs["2016b_TrackerMuonOrGlobalMuon"].clone(
    bins=[
        { 'var' : 'probe_eta' , 'type': 'float', 'bins': [round(i/20.,2) for i in range(-48,49)], 'title':'#eta' },
        { 'var' : 'probe_pt' , 'type': 'float', 'bins': [35, 70], 'title':'p_{T} [GeV]' },
    ],
...
Configs["2016b_TrackerMuonOrGlobalMuon_mt"]=tnpConfig(
    data="/data9/Users/wonjun/public/TnP_Trees/TnPTreeZ_21Feb2020_UL2016_SingleMuon_Run2016FGHv1.root",
    sim="/data9/Users/wonjun/public/TnP_Trees/TnPTreeZ_106XSummer19_UL16RECO_DYJetsToMuMu_M50_powhegMiNNLO_WithWeights.root",
    sim_weight='(pair_genWeight/2358.)',
    sim_maxweight=10000.,
    sim_genmatching='mcTrue',
    sim_genmass="mcMass",
    tree='tpTree/fitter_tree',
    mass="mass",
    bins=[
        { 'var' : 'eta' , 'type': 'float', 'bins': [round(i/20.,2) for i in range(-48,49)], 'title':'#eta' },
        { 'var' : 'pt' , 'type': 'float', 'bins': [35, 70], 'title':'p_{T} [GeV]' },
    ],
    preselection='Track_HP',
    expr='Track_HP && tag_IsoMu24 && tag_pt>27 && tag_charge*charge < 0 && pair_deltaR > 0.3 && tag_mt<40',
    test='TM||Glb',
    hist_nbins=98,
    hist_range=(52,150),
    method='fit',
    ...
    systematic=[
        ...
        [{'title':'altsig2','fit_parameter':fit_altsig2}],
        [{'title':'alttag','expr.replace':('tag_pt>','tag_pt>8+')}],
        [{'title':'altmc','sim.replace':[('TnPTreeZ_106XSummer19_UL16RECO_DYJetsToMuMu_M50_powhegMiNNLO_WithWeights','TnPTreeZ_106XSummer19_UL16RECO_DYJetsToLL_M50_MadgraphMLM_WithWeights')]}],
        [{'title':'PUweight_up'}],
        [{'title':'prefireweight'}],
        [{'title':'zptweight'}],
        [{'title':'z0weight'}],
