...
### use TrakerMuon||GlobalMuon instead of TrakerMuon for RECO; wider binning for RECO and tracking;
############## samples ################
...
binnings={
    ...
    'RECO':[            ### For RECO
        { 'var' : 'probe_eta' , 'type': 'float', 'bins': [round(i/10.,2) for i in range(-24,25)], 'title':'#eta' },
        { 'var' : 'probe_pt' , 'type': 'float', 'bins': [35, 50], 'title':'p_{T} [GeV]' },
    ],
...
fit_nominal_reco = [
    "HistPdf::sigPass(x,histPass_genmatching,2)",
    "HistPdf::sigFail(x,histPass_genmatching,2)",
    "RooCMSShape::bkgPass(x,acmsP[50.,40.,80.],betaP[0.1,0.01,0.25],gammaP[0.05, 0.0001, 0.2],peakP[90.0])",
    "RooCMSShape::bkgFail(x,acmsF[50.,40.,80.],betaF[0.1,0.01,0.25],gammaF[0.05, 0.0001, 0.2],peakF[90.0])",
]
fit_altsig_reco = [
    "HistPdf::sigPhysPass(x,histPass_genmatching,2)",
    "HistPdf::sigPhysFail(x,histPass_genmatching,2)",
    "Gaussian::sigResPass(x,meanGaussP[0.0,-5.0,5.0],sigmaP[0.02,0.02,4.0])",
    "Gaussian::sigResFail(x,meanGaussF[0.0,-5.0,5.0],sigmaF[0.02,0.02,4.0])",
    "FCONV::sigPass(x, sigPhysPass , sigResPass)",
    "FCONV::sigFail(x, sigPhysFail , sigResFail)",
...
fit_altsig2_reco = [
    "HistPdf::sigPhysPass(x,histPass_genmatching_genmass,2)",
    "HistPdf::sigPhysFail(x,histPass_genmatching_genmass,2)",
    "Gaussian::sigResPass(x,meanGaussP[0.0,-5.0,5.0],sigmaP[0.02,0.02,4.0])",
    "Gaussian::sigResFail(x,meanGaussF[0.0,-5.0,5.0],sigmaF[0.02,0.02,4.0])",
    "FCONV::sigPass(x, sigPhysPass , sigResPass)",
    "FCONV::sigFail(x, sigPhysFail , sigResFail)",
    "Fit sigPass histPass_genmatching",
    "Fit sigFail histPass_genmatching",
    "SetConstant meanGaussP sigmaP meanGaussF sigmaF",
    "RooCMSShape::bkgPass(x,acmsP[50.,40.,80.],betaP[0.1,0.01,0.25],gammaP[0.05, 0.0001, 0.2],peakP[90.0])",
    "RooCMSShape::bkgFail(x,acmsF[50.,40.,80.],betaF[0.1,0.01,0.25],gammaF[0.05, 0.0001, 0.2],peakF[90.0])",
]
...
config_id=tnpConfig(
    ...
    mass="pair_mass_cor",
    bins=binnings['ID'],
    expr='tag_IsoMu24 && tag_pt_cor>27 && tag_isTight && tag_PFIsoTight && tag_q*probe_q<0 && (probe_isTracker||probe_isGlobal)',
    ...
    systematic=[
        ...
        [{'title':'altfactoring','expr.replace':('(probe_isTracker||probe_isGlobal)','probe_isLoose')}],
    ]
)
...
config_tracking=config_id.clone(
    ...
    expr='tag_IsoMu24 && tag_pt_cor>35&&tag_pt_cor<50 && tag_isTight && tag_PFIsoTight && tag_q*probe_q<0 && probe_isSA_unique',
    test='probe_isGlobal',
    systematic=[
        [{'title':'altbkg','fit_parameter':fit_altbkg_reco}],
        [{'title':'altbkg2','fit_parameter':fit_altbkg2_reco}],
        [{'title':'altsig','fit_parameter':fit_altsig_reco}],
        [{'title':'altsig2','fit_parameter':fit_altsig2_reco}],
        [{'title':'alttag','expr.replace':('tag_pt_cor>35','tag_pt_cor>30')}],
        [{'title':'altmc','sim.replace':[('DYJetsToMuMu_M-50_massWgtFix_TuneCP5_13TeV-powhegMiNNLO-pythia8-photos','DYJetsToLL_M-50_TuneCP5_13TeV-madgraphMLM-pythia8'),('DYJetsToMuMu_M-50_TuneCP5_13TeV-powhegMiNNLO-pythia8-photos','DYJetsToLL_M-50_TuneCP5_13TeV-madgraphMLM-pythia8')]}],
        [{'title':'PUweight_up','sim_weight.replace':('PUweight','PUweight_up')},{'title':'PUweight_down','sim_weight.replace':('PUweight','PUweight_down')}],
        [{'title':'prefireweight_up','sim_weight.replace':('prefireweight','prefireweight_up')},{'title':'prefireweight_down','sim_weight.replace':('prefireweight','prefireweight_down')}],
        [{'title':'zptweight','sim_weight.replace':('*zptweight','')}],
...
Configs["2017_GlobalMuon"]=config_tracking.clone(
    ...
    expr=config_tracking.expr.replace('tag_IsoMu24','tag_IsoMu27'),
)    
...
config_reco=tnpConfig(
    ...
    expr='probe_isHighPurity > 7 && tag_hltL3crIsoL1sSingleMu22L1f0L2f10QL3f24QL3trkIsoFiltered0p07 && tag_pt>35&&tag_pt<50 && tag_CutBasedIdTight && tag_PFIsoTight && tag_charge*probe_charge < 0 && pair_dR > 0.3 && abs(pair_dz)<0.05',
    ...
    systematic=[
        ...
        [{'title':'altsig','fit_parameter':fit_altsig_reco}],
        [{'title':'altsig2','fit_parameter':fit_altsig2_reco}],
        [{'title':'alttag','expr.replace':('tag_pt>35','tag_pt>30')}],
        [{'title':'altmc','sim.replace':[('DYJetsToMuMu_M-50_massWgtFix_TuneCP5_13TeV-powhegMiNNLO-pythia8-photos','DYJetsToLL_M-50_TuneCP5_13TeV-madgraphMLM-pythia8'),('DYJetsToMuMu_M-50_TuneCP5_13TeV-powhegMiNNLO-pythia8-photos','DYJetsToLL_M-50_TuneCP5_13TeV-madgraphMLM-pythia8')]}],
        [{'title':'PUweight_up','sim_weight.replace':('puWeight','puWeight_up')},{'title':'PUweight_down','sim_weight.replace':('puWeight','puWeight_down')}],
        ...
        [{'title':'countbroad','count_range':(70,112)},
         {'title':'countnarrow','count_range':(86,96)}],
        [{'title':'massbinmore','hist_nbins':192},
         {'title':'massbinless','hist_nbins':48}],
        [{'title':'genmatching'}], ## not implemented
        [{'title':'fitting'}], ## only for counting
        [{'title':'altfactoring','test':'probe_isLoose'}],
    ]
)
...
Configs["2017_TrackerMuonOrGlobalMuon"]=config_reco.clone(
    ...
    expr=config_reco.expr.replace(IsoMu24_2018,IsoMu27_2017),
)
